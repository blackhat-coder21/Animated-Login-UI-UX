...
import com.example.chatapp.databinding.ActivityRegisterTwitterBinding
import com.google.firebase.auth.FirebaseAuth

class RegisterTwitter : AppCompatActivity() {

    lateinit var firebaseAuth: FirebaseAuth
    lateinit var binding: ActivityRegisterTwitterBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_register_twitter)

        supportActionBar?.hide()
        getWindow().addFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN);

        binding = ActivityRegisterTwitterBinding.inflate(layoutInflater)
